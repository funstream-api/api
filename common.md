...
*Вернёт ошибку если пользователь с таким именем уже существует, имя пользователя недопустимо или не была пройдена капча.*
...
##### [`POST` `P` `/api/support?name=<streamer_name>`](https://funstream.tv/api/support?name=<streamer_name>)(заменить на json?)
**запрос**
```js
[]
```
