...
*Ответ ошибка в случае если пользователь с таким именем или id не найден, либо если и имя и id переданы как `null`.*
...
```js
{
    ...
    roles: <obj> { // Роли пользователя
        ...
        citizen: <int> // Сила голоса гражданской модерации, 0 по умолчанию
    }
}
```
...
*Ответ ошибка если пользователь с таким именем и паролем не найден.*
...
*Ответ ошибка если пользователь с таким именем уже существует, либо если не была пройдена капча.*
...
```
*В данный момент метод некорректно отдаёт NULL для всех настроек кроме переданных для изменения.*  
...
*Ответ ошибка если токена авторизации нету или он ошибочен, или если указаны ошибочные настройки.*
...
*Ответ ошибка если категория с таким `id` или `slug` не найдена.*
...
```
*Ответ ошибка если стрима с данным id или именем стримера не существует.*
...
```
*Ответ ошибка если хотя бы один из переданных идентификаторов неверен.*
...
```js
{
    ...
    category: <obj|null> { // Категория, любой из параметров
        id: <int|null>, // Идентификатор категории
        slug: <string|null> // Короткое имя категории
    }
}
...
```
*Вернёт список стримов, отсортированных по текущему рейтингу. Поле weight у категорий(текущей и дочерних) это суммарный теукщий рейтинг стримов в этой категории. Сортировка дочерних категорий по этому параметру.*  
...
```
*Вернёт количество активных(онлайн) элементов в подписка у пользователя.*  
...
```
*Вернёт список всех имеющихся подписок на переданный тип контента. В данный момент работает неверно.*  
*Вернёт ошибку если передан наверный тип контента, пользователь не залогинен или передан неверный токен.*
...
**(!)** Система игноров в работе, методы могут работать некорректно или не работать вовсе.
