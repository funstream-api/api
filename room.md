...
```js
{
    ...
    playlistPosition: <int>, // Позиция трансляции в плейлисте, от 0
...
{
    roomId: <int>, // Идентификатор комнаты
    position: <int> // Позиция трансляции в плейлисте комнаты, от 0
}
```
**ответ**
```js
...
[
    {
      ...
      position: <int>, // Позиция в плейлисте, от 0
      url: <string>, // Ссылка на трансляцию
...
{}
```
*Вернёт ошибку если указан неверный идентификатор комнаты, уже имеется запрос от пользователя или у пользователя есть доступ в эту комнату.*
...
```js
{
    inviteId: <int> // Идентификатор запроса
}
```
**ответ**
```js
...
```js
[
    <obj>, // Данные запроса, объект из ответа /api/room/invite/status
    ...
]
```
...
```js
{
    ...
    roomId: <int>, // Идентификатор комнаты
    blacklisted: <bool>, // Статус наличия запроса в чёрном списке
