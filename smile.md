...
[
    {
        ...
        position: number; // Позиция(порядок для сортировки)
        width: number; // Ширина картинки в пикселях
        height: number; // Высота картинки в пикселях
        level: number; // Требуемый уровень пользователя для мс смайлов, 0 для остальных, DEPRECATED(см changelog)
        masterStreamerLevel: number; // Уровень смайла мастерстримера, 0 для общедоступного смайла
        siteLevel: number; // Уровень общедоступного смайла, 0 для смайла мс
