...
[
    {
        ...
        level: <int>, // Требуемый уровень пользователя для мс смайлов, 0 для остальных
        user: <obj|null>, // Мастерстример к которому относится смайл, объект из ответа /api/user
...
*Все смайлы, кроме относящихся к мастерстримерам, разбиты по группам с указанной позицией внутри группы. Для смайлов мастерстримеров `tab` может быть произвольный, указаны `level` и данные мастерстримера в `user`.*  
